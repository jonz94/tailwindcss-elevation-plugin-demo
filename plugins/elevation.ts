...
function getUmbra(zValue: ZValue) {
  ...
  return `${umbarMap[zValue]} ${shadowColor}`
}
...
function getPenumbra(zValue: ZValue) {
  ...
  return `${penumbraMap[zValue]} ${shadowColor}`
}
...
function getAntumbra(zValue: ZValue) {
  ...
  return `${antumbraMap[zValue]} ${shadowColor}`
}
...
function getUtilites() {
  const elevationUtilitiesEntries = getArrayFrom0To24().map((i) => {
    const key = `.elevation-${i}`
    const value = {
      'box-shadow': [getUmbra(i), getPenumbra(i), getAntumbra(i)].join(', '),
    }
